...
async def execute_requested_tool(
    ...
    *,
    ...
    if property_tool == PropertyToolName.ROOM_BOOKING:
        try:
            booking_input = RoomBookingToolInput.model_validate_json(tool_call.arguments)
        ...
        try:
            return await execute_room_booking_tool(
                booking_input,
                context=context,
...
def get_tool_trace_details(
    ...
    if isinstance(
        ...
        return (
            ROOM_BOOKING_TOOL_LABEL,
            (1 if result.status == "confirmed" else 0),
        )
...
async def run_hotel_assistant(
    *,
    ...
    assistant_session = await get_or_create_assistant_session(
        context.session,
    ...
    history = get_conversation_history(assistant_session)
